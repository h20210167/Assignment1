...
#define select_align _IOR(magic_no, 2, char *)
...
int main()
{
	...
	if(channel <0 || channel>7 || (align != 'r' && align != 'l' && align != 'R'&&
	                                                              align != 'L'))
  	...
  	if (ioctl(fd, select_channel, channel) < 0) 
	{
    	...
    	if (ioctl(fd, select_align, align) < 0)
    	 {
	...
	
	{
		if(align == 'l')
		{
		  printf("left aligned number %u \n", data);
		  printf("original number %u \n", data/16);
		}
		
	        else
		{
		  printf("right aligned number %u \n", data);
		  printf("original number %u \n", data/16);
        	}
